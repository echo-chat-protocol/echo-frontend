  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="anim-fade-up mt-10 ticker-mask overflow-hidden" style={{ animationDelay: "480ms" }}>
...
const CONVERSATIONS = [
  {
    contact: { name: "Aïsha · @whisper", fp: "4F:9A:1C:…:E2 · verified ✓" },
    history: [
      { from: "them", text: "The drop is ready. Sending the keyshare now." },
      { from: "me", text: "Got it. Shamir threshold 3-of-5, right?" },
      { from: "them", text: "✅ 3-of-5. Shards encrypted with your pub: ed25519:7HK…" },
    ],
    newMsg: "Sealed and stored. Burning ephemeral RAM.",
    sidebar: [
      { name: "🛡 Crypto Council", last: "All 3 keys rotated", active: false },
      { name: "Aïsha", last: "ok 🔐", active: true },
      { name: "Ops • Red Team", last: "drop the .pcap", active: false },
    ],
  },
  {
    contact: { name: "Ops • Red Team", fp: "A1:B2:3C:…:D4 · verified ✓" },
    history: [
      { from: "me", text: "Is the relay still active?" },
      { from: "them", text: "Yeah, sub-40ms. All traffic sealed." },
      { from: "me", text: "Perfect. Initiating key exchange now." },
    ],
    newMsg: "Drop the .pcap when you're ready.",
    sidebar: [
      { name: "🛡 Crypto Council", last: "All 3 keys rotated", active: false },
      { name: "Aïsha", last: "ok 🔐", active: false },
      { name: "Ops • Red Team", last: "drop the .pcap", active: true },
    ],
  },
  {
...
function HeroAppPreview() {
  ...
  const [phase, setPhase] = React.useState("idle"); // idle | typing | sent
              ...
              {conv.sidebar.map((r) => (
                ...
                >
                  <div>
                    ...
                    <div className="text-[11px] text-[#a8a8b8] truncate max-w-[150px]">{r.last}</div>
                  </div>
                  {r.active && <span className="h-1.5 w-1.5 rounded-full bg-[#a855f7] flex-shrink-0" />}
                </div>
              ))}
            </div>
          </aside>
