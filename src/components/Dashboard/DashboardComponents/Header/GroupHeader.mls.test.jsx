...
// socket fallback). Mock it so these MLS tests exercise the membership flow
...
describe('GroupHeader MLS membership updates', () => {
        ...
        if (event === 'openGroup') {
          callback?.({
            ...
            group: {
              ...
              mlsEnabled,
              epoch: 2,
              cipherSuite: mlsEnabled ? 'Echo-MLS-TreeKEM/X25519_AES256GCM_SHA256' : null,
            },
  ...
  it('keeps legacy group membership changes free of MLS commit and welcome events', async () => {
    ...
    expect(socket.emit.mock.calls.some(([eventName]) => eventName === 'sendGroupCommit')).toBe(
      false
    )
    expect(socket.emit.mock.calls.some(([eventName]) => eventName === 'sendGroupWelcome')).toBe(
      false
    )
  })
  ...
  it('updates the group description through the profile modal', async () => {
    ...
    const descriptionField = container.querySelector(
      'textarea[placeholder="Add a description for this group"]'
    )
    await act(async () => {
