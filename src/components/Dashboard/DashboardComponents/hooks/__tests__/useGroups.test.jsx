...
describe('useGroups hook', () => {
  ...
  it('does not replace a real group name with the generic placeholder', async () => {
    const userId = 'U3'
    const storageKey = `groups-${userId}`
    localStorage.setItem(storageKey, JSON.stringify([{ groupId: 'G1', name: 'Project Alpha' }]))
    ...
    await act(async () => {
      api.upsertGroup({ groupId: 'G1', name: 'Group' }, { text: 'latest', timestamp: 't1' })
      await flush()
    })
    ...
    await act(async () => {
      api.setAllGroups([{ groupId: 'G1', name: 'Group' }])
      await flush()
    })
