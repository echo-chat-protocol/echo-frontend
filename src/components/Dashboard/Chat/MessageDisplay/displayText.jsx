      ...
      {messages.map((message, index) => (
        <div key={message._id} className='space-y-2'>
          ...
          {message.messageType === 'call_event' ? (
            ...
            >
              <div
                className={`message-bubble max-w-xs md:max-w-md lg:max-w-lg xl:max-w-xl rounded-lg p-3 ${
                  message.userId === currentUserId
                    ? 'outgoing bg-indigo-600 text-white rounded-br-none'
                    : 'incoming bg-gray-700 text-white rounded-bl-none'
                }`}
