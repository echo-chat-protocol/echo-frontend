...
describe('wrapGroupKey / unwrapGroupKey', () => {
  ...
  it('wrapped envelope contains non-empty encryptedB64 and ephPubB64 (HPKE format — no nonceB64)', async () => {
    ...
    // HPKE embeds the nonce into the key schedule; it is not transmitted.
    expect(wrapped.nonceB64).toBeUndefined()
  })

  it('two calls produce different ephemeral keys and ciphertexts (non-deterministic HPKE encap)', async () => {
    ...
    // Each seal generates a fresh ephemeral keypair, so enc and ct differ.
    expect(w1.ephPubB64).not.toBe(w2.ephPubB64)
    expect(w1.encryptedB64).not.toBe(w2.encryptedB64)
  })

  it('backward-compat: unwrap decrypts a legacy DH+HKDF+AES envelope (nonceB64 present)', async () => {
    const { privB64, pubB64 } = makeKeypair(0x20)
    const aad = makeCommitAadBytes('group-legacy', 1)

    // Construct a legacy envelope manually using the same primitives the old code used.
    const WRAP_INFO_BYTES = new TextEncoder().encode('EchoMLS/v1/WelcomeWrap')
    const groupKeyBytes = base64ToBytes(GROUP_KEY_B64)
    const recipientPub = base64ToBytes(pubB64)

    const ephPriv = x25519.generate_private_ephemeral_key(new Uint8Array(32).fill(0x30))
    const ephPub = x25519.generate_public_ephemeral_key(ephPriv)
    const dh = x25519.diffie_hellman(ephPriv, recipientPub)
    const wrapKey = x25519.hkdf_derive(dh, new Uint8Array(0), WRAP_INFO_BYTES, 32)
    const nonce = new Uint8Array(12).fill(0x42)

    const aesModule = await import('@mascaro101/echo-protocol/aes.js')
    const encrypted = aesModule.encrypt_aad_bytes(groupKeyBytes, wrapKey, nonce, aad)

    const legacyEnvelope = {
      encryptedB64: bytesToBase64(encrypted),
      ephPubB64: bytesToBase64(ephPub),
      nonceB64: bytesToBase64(nonce),
    }

    const recovered = await unwrapGroupKey(legacyEnvelope, privB64, aad)
    expect(recovered).toBe(GROUP_KEY_B64)
  })
})
...
describe('makePathSecretAadBytes', () => {
  ...
  it('is deterministic for the same nodeIndex', () => {
    expect(Buffer.from(makePathSecretAadBytes(3))).toEqual(Buffer.from(makePathSecretAadBytes(3)))
  })
})
