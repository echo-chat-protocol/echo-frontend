...
vi.mock('../dr.js', () => ({
  ...
  continueDoubleRatchetChain: vi.fn(async () => ({
    receivingChainKey: new Uint8Array(32).fill(1),
    newRootKey: new Uint8Array(32).fill(1),
  })),
...
function snapshot(userId) {
  ...
  const snapMap = (m) =>
    Object.fromEntries(
      [...m.entries()].map(([k, v]) => [k, v instanceof Uint8Array ? toHex(v) : v])
    )
  const snapObjMap = (m) =>
    Object.fromEntries([...m.entries()].map(([k, v]) => [k, JSON.parse(JSON.stringify(v))]))
  return {
...
describe('protocol adversarial: fail-closed without state desync', () => {
  ...
  it('rejects if message header fields (AAD) are tampered; does not advance Nr/CKr', async () => {
    ...
    await asUser(bob, async () => {
      ...
      await expect(
        decryptIncomingMessage(msg, nonce, bob, alice, new Uint8Array(32), {}, null)
    ...
    await asUser(bob, async () => {
      ...
      await expect(
        decryptIncomingMessage(tampered, nonce, bob, alice, new Uint8Array(32), {}, null)
      ).rejects.toThrow()
    })
  ...
  it('ignores replay (same n after Nr advanced) without changing state', async () => {
    ...
    await asUser(bob, async () => {
      const nonce = base64ToArrayBuffer(msg.nonce)
      const out1 = await decryptIncomingMessage(
        msg,
        nonce,
        bob,
        alice,
    ...
    await asUser(bob, async () => {
      const nonce = base64ToArrayBuffer(msg.nonce)
      const out2 = await decryptIncomingMessage(
        msg,
        nonce,
        bob,
        alice,
