...
describe('groupCryptoProvider welcomes', () => {
  ...
  it('buildInitialWelcomes signs each welcome with the creator leaf signing key metadata', async () => {
    const { welcomes } = await buildCreatorAndWelcomes(
      'group-welcome-sig-1',
      ALICE_BOB_ROSTER,
      ALICE_BOB_INIT_KEYS
    )
    ...
    const bobWelcome = welcomes.find((welcome) => welcome.recipientUserId === 'bob')
    expect(bobWelcome.senderLeafIndex).toBe(0)
    expect(typeof bobWelcome.senderSigningPubKeyB64).toBe('string')
    expect(typeof bobWelcome.signature).toBe('string')
    expect(bobWelcome.signature.length).toBeGreaterThan(0)
  })
})
...
describe('groupCryptoProvider application messages', () => {
  ...
  it('advances the receiver state after each decrypted application message', async () => {
    ...
    expect(secondDecrypted.newState.applicationSecretB64).toBe(
      secondEncrypted.newState.applicationSecretB64
    )
  })
...
describe('groupCryptoProvider commits', () => {
  ...
  it('resets sender generation to 0 after a remove commit even when the sender had already sent messages', async () => {
    ...
    const creatorAfterMessage = (
      await encryptApplicationMessage({
        state: creatorState,
