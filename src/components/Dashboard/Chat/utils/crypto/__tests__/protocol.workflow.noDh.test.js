...
const km = vi.hoisted(() => ({
  ...
  getRootKey: vi.fn(
    async (userId, targetUserId) => state.rootKey.get(k(userId, targetUserId)) ?? null
  ),
  ...
  getEphemeralData: vi.fn(
    async (userId, targetUserId) => state.eph.get(k(userId, targetUserId)) ?? null
  ),
  ...
  getSendingChainKey: vi.fn(
    async (userId, targetUserId) => state.sendingChainKey.get(k(userId, targetUserId)) ?? null
  ),
  setSendingChainKey: vi.fn(async (userId, targetUserId, ck) => {
    state.sendingChainKey.set(
      k(userId, targetUserId),
  ...
  getReceivingChainKey: vi.fn(
    async (userId, targetUserId) => state.receivingChainKey.get(k(userId, targetUserId)) ?? null
  ),
  setReceivingChainKey: vi.fn(async (userId, targetUserId, ck) => {
    state.receivingChainKey.set(
      k(userId, targetUserId),
      ck instanceof Uint8Array ? ck : new Uint8Array(ck)
    )
  }),
  ...
  getCurrentSendingNumber: vi.fn(
    async (targetUserId) => state.ns.get(k(activeUserId, targetUserId)) ?? 0
  ),
  ...
  getCurrentReceivingNumber: vi.fn(
    async (targetUserId) => state.nr.get(k(activeUserId, targetUserId)) ?? 0
  ),
  ...
  getPreviousSendingNumber: vi.fn(
    async (targetUserId) => state.pn.get(k(activeUserId, targetUserId)) ?? 0
  ),
  ...
  getSkippedMessages: vi.fn(
    async (targetUserId) => state.skipped.get(k(activeUserId, targetUserId)) ?? {}
  ),
...
async function sendOnExistingChain({ from, to, text }) {
  return asUser(from, async () => {
    ...
    const nextChainKey = material.slice(32, 64)
    const nonce = material.slice(64, 76)
