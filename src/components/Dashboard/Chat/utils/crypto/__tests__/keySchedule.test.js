...
const wasm = vi.hoisted(() => ({
  ...
  hkdf_extract: vi.fn((salt, ikm) => {
    ...
    for (let i = 0; i < out.length; i++) {
      out[i] =
        ((saltBytes[i % saltBytes.length] || 0) ^ (ikmBytes[i % ikmBytes.length] || 0) ^ 0x5a) &
        0xff
    }
    return out
  }),
  hkdf_expand: vi.fn((prk, info, length) => {
    ...
    for (let i = 0; i < length; i++) {
      out[i] =
        ((prkBytes[i % prkBytes.length] || 0) + (infoBytes[i % infoBytes.length] || 0) + i) & 0xff
    }
    return out
  }),
}))
...
describe('keySchedule expandWithLabel', () => {
  it('initializes the WASM module and passes the encoded HKDFLabel into hkdf_expand', async () => {
    ...
    expect(wasm.hkdf_expand).toHaveBeenCalledWith(secret, encodeHKDFLabel(32, 'epoch', context), 32)
