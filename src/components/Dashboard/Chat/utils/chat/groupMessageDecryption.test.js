...
describe('decryptIncomingGroupMessage', () => {
  ...
  it('supports decryptApplicationMessage returning raw plaintext bytes', async () => {
    ...
    decryptApplicationMessageMock.mockResolvedValue(new TextEncoder().encode('raw bytes plaintext'))
  ...
  it('accepts messages with encryptedSenderDataB64 instead of headerB64 (item #3)', async () => {
    const localState = { groupId: 'group-1', senderGenerations: {} }
    const advancedState = { groupId: 'group-1', senderGenerations: { 0: 1 } }
    loadGroupStateMock.mockResolvedValue(localState)
    decryptApplicationMessageMock.mockResolvedValue({
      plaintextBytes: new TextEncoder().encode('sender-data message'),
      newState: advancedState,
    })
    saveGroupStateMock.mockResolvedValue({ ...advancedState })
    ...
    const result = await decryptIncomingGroupMessage({
      message: {
        ...
        userId: 'bob',
        contentType: 'application',
        encryptedSenderDataB64: 'enc-sender-data',
        ciphertextB64: 'cipher-sd',
    ...
    expect(decryptApplicationMessageMock).toHaveBeenCalledWith({
      ...
      encryptedSenderDataB64: 'enc-sender-data',
      header: null,
      ciphertext: 'cipher-sd',
      includeNewState: true,
    })
    expect(result.formattedMessage.text).toBe('sender-data message')
  })

  it('passes both encryptedSenderDataB64 and headerB64 when both are present (messageFlow picks)', async () => {
    const currentState = { groupId: 'group-1' }
    decryptApplicationMessageMock.mockResolvedValue({
      plaintextBytes: new TextEncoder().encode('both fields'),
      newState: currentState,
    })
    ...
    await decryptIncomingGroupMessage({
      message: {
        ...
        userId: 'bob',
        contentType: 'application',
        encryptedSenderDataB64: 'enc-sd-both',
        headerB64: 'header-both',
        ciphertextB64: 'cipher-both',
      },
      userId: 'alice',
      currentState,
    })
  ...
  it('passes encryptedSenderDataB64 to consumePendingOutgoingGroupMessage for self-authored messages', async () => {
    ...
    await decryptIncomingGroupMessage({
      message: {
        ...
        userId: 'alice',
        contentType: 'application',
        encryptedSenderDataB64: 'enc-sd-self',
        ciphertextB64: 'cipher-self',
      },
      userId: 'alice',
      username: 'Alice',
  ...
  it('hasAppMessage is false when ciphertextB64 is missing (no decryption attempted)', async () => {
    const currentState = { groupId: 'group-1' }
    ...
    const result = await decryptIncomingGroupMessage({
      message: {
        ...
        userId: 'bob',
        contentType: 'application',
        encryptedSenderDataB64: 'enc-sd-no-ct',
        // ciphertextB64 intentionally absent
      },
      userId: 'alice',
      currentState,
    })
    ...
    expect(decryptApplicationMessageMock).not.toHaveBeenCalled()
    expect(result.formattedMessage.text).toBe('')
  })
})
