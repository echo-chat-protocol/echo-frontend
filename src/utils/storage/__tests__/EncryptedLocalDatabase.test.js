...
describe('DR root keys', () => {
  ...
  it('throws on invalid root key length', async () => {
    await expect(db.storeRootKey('bob', new Uint8Array(16))).rejects.toThrow(
...
describe('DR sending chain keys', () => {
  ...
  it('throws on invalid length', async () => {
    await expect(db.storeSendingChainKey('bob', new Uint8Array(31))).rejects.toThrow(
      'Invalid sending chain key length'
    )
  })
})
...
describe('DR receiving chain keys', () => {
  ...
  it('throws on invalid length', async () => {
    await expect(db.storeReceivingChainKey('bob', new Uint8Array(33))).rejects.toThrow(
      'Invalid receiving chain key length'
    )
  })
})
...
describe('MLS group state', () => {
  ...
  it('resets only the selected user local encrypted records', async () => {
    const alice = uid()
    const carol = uid()
    const resetDb = new EncryptedLocalDatabase()

    await resetDb.createUser(alice, 'secret')
    await resetDb.storeMessage('bob', {
      _id: 'm1',
      text: 'hello',
      createdAt: '2025-01-01T00:00:00Z',
    })
    await resetDb.storeMlsGroupState('g1', { epoch: 1 })
    resetDb.lock()

    await resetDb.createUser(carol, 'secret')
    await resetDb.storeMessage('dave', {
      _id: 'm2',
      text: 'keep',
      createdAt: '2025-01-01T00:00:00Z',
    })
    ...
    await resetDb.resetUser(alice)
    expect(await resetDb.userExists(alice)).toBe(false)
    expect(await resetDb.userExists(carol)).toBe(true)

    await resetDb.unlock(carol, 'secret')
    expect(await resetDb.getMessages('dave')).toHaveLength(1)
  })
})
